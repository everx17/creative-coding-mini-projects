    ...
    def genome_to_links(gdicts):
        ...
        for gdict in gdicts:
            ...
            link = URDFLink(name=link_name, 
                            ...
                            link_radius=gdict["link-radius"]+0.005, 
                            ...
                            joint_origin_xyz_2=gdict["joint-origin-xyz-2"]+0.5,
                            joint_origin_xyz_3=gdict["joint-origin-xyz-3"]+0.5,
        ...
        for gene in new_genome:
            for i in range(len(gene)):
                if random.random() < rate:
                    scaled_amt = (random.random() - 0.5) * 2 * amount
                    gene[i] += scaled_amt
                    # if mutation went out of range, apply inverse of it
                    if gene[i] >= 1.0 or gene[i] < 0.0:
                        gene[i] = gene[i] - 2 * scaled_amt
                    # and still check bounds in case of too high amount
                    if gene[i] >= 1.0:
                        gene[i] = 0.9999
                    if gene[i] < 0.0:
                        gene[i] = 0.0
